...
exports.activity = function(data) {
  return {
    ...
    title: function() {
      var titles = _.as_array(this.data.title || this.data.name);

      // pick @xml-lang='en' first
      titles = _.sortBy(titles, function(a){
        return a['@xml:lang'] == 'en' ? 0 : 1;
      });

      var title = textVal(titles[0]);
    ...
    description: function() {
      var descriptions = _.as_array(this.data.description);

      // pick @xml-lang='en' first
      var description = _.sortBy(descriptions, function(a){
        return a['@xml:lang'] == 'en' ? 0 : 1;
      })[0];

      return textVal(description)
        .replace(/^\s+|\s+$/g, "")
        .replace(/\r\n|\r|\n/g, "<br>");
    },
    ...
    contactInfo: function(){
      return _.as_array(this.data['contact-info']).map(_.without_xml_attrs);
    },
    ...
    transactions: function(types){
      var txs = _.as_array(this.data.transaction);

      // if we only want transactions of
      // a particular set of types (eg 'IF D')
      if(types){
        types = types.split(' ');
        txs = _.filter(txs, function(t){
          if(!t["transaction-type"]) return false;
          
          var code = t["transaction-type"]['@code'];
          return _.contains(types, code);
        });
      }
      // console.log(this.data.transaction);
      return _.as_array(txs);
    },

    budgets: function(){
      return _.as_array(this.data.budget);
    },
    ...
    locations: function() {
      return _(this.data.location).chain()
        ...
        .map(function(l) {
          return {
            name: textVal(l.name),
            description: textVal(l.description),
    ...
    recipientCountries: function(){
      return _(this.data['recipient-country']).chain()
        .as_array()
        .map(function(country){
          return {
            code:country['@code'],
            value:parseFloat(country['@iati-ad:project-value']) || 0,
            name:textVal(country)
          };
        })
        .value();
    },
    ...
    documents: function() {
      return _(this.data['document-link']).chain()
        .as_array()
        .map(function(d) {
          return {title: textVal(d.title).replace(/\..+$/g, " ") || "Unnamed", url: d['@url']};
        })
        .value();
    },
