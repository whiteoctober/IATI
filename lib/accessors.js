...
var textVal = function(node) { 
  return ((node && node['#text']) ? node['#text'] : (node.substr ? node : "") ) || ""; 
};
  ...
  return {
    ...
    participators: function() {
      return _(this.data['participating-org']).chain()
        .as_array()
        .map(function(o) { return {name: o["#text"], id: o['@ref'], funder: !!o["@iati-ad:funder"]}; })
        .value();
    },
    ...
    dates: function() {
      ...
      _(dateList).each(function(d) {
        ...
        dates[type[0]][type[1]] = d['#text'] ? new Date(Date.parse(d['#text'])) : false;
      });
      return dates;
    },
